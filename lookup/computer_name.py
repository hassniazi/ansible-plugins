...
class LookupModule(LookupBase):
    def run(self, terms, variables=None, boto_profile=None, aws_profile=None,
            ...
            product_short_code=None, os_type=None):
        '''
        ...
        filter_string = '{}-{}-*'.format(product_short_code.upper(), os_type[:1].upper())
        ...
        if len(computer_numbers) > 0:
            ...
            set_full_range = set(range(1, 999))
            set_diff = set_full_range - set_numbers
            try:
                num = min(set_diff)
            except ValueError:
                raise AnsibleError('Too many instances')
            number_string = format(num, '03')
        computer_name = '{}-{}-{}'.format(product_short_code.upper(), os_type[:1].upper(), number_string)
