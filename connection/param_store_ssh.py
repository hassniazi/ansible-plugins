...
class Connection(ConnectionBase):
    transport = 'param_store_ssh'
    def __init__(self, *args, **kwargs):
        super(Connection, self).__init__(*args, **kwargs)
        logging.basicConfig(
            format='%(asctime)s %(message)s',
            datefmt='%d/%m/%Y %I:%M:%S %p',
            filename='/tmp/param_store_ssh.log',
            level=logging.INFO
        )
        #logging.info(dir(self._play_context))
        logging.info(self._options)
        self.host = self._play_context.remote_addr
        self.port = self._play_context.port
        self.user = self._play_context.remote_user
        self.control_path = None
        self.control_path_dir = '~/.ansible/cp'
        self._load_name = 'param_store_ssh'
        self.key_name = self._play_context.private_key_file
        logging.info(self._play_context.private_key_file)
        self.key_path = self._private_key_from_param_store()
        self._play_context.private_key_file = self.key_path
    
    def _private_key_from_param_store(self):
        '''
        '''
        ssm = self._boto3_conn()
        key_param_name = 'KEYPAIR_{}'.format(self.key_name)
        response = ssm.get_parameter(
            Name=key_param_name,
            WithDecryption=True
        )
        private_key = response['Parameter']['Value']
        path = '/tmp/{}.pem'.format(self.key_name)
    ...
    def _boto3_conn(self):
        region = os.environ.get('AWS_REGION', DEFAULT_AWS_REGION)
        try:
            connection = boto3.session.Session(profile_name=None).client('ssm', region)
